...
## 🎯 Strategic Thesis
Demonstrate to the Eclipse SDV community and advanced systems engineers that uProtocol is the mandatory semantic layer for Software-Defined Vehicles (SDVs). Prove that while raw transport layers (like local Unix Domain Socket) move bytes perfectly, they are inherently blind to automotive application semantics, data isolation, and location transparency.
...
- [x] **1.1 Establish the Baseline (Real Code):** Present the working multi-binary Cargo layout; rename `up-client` → `up-battery-telemetry-publisher`, `up-server` → `up-telemetry-subscriber`. Unix Domain Socket length-framed `UMessage` pub/sub. Code: `phases/01_raw_sockets/`.
...
- [x] **1.3 Demonstrate "Deep Water" (Pseudo-Code Friction):** Markdown pseudo-code showing accidental in-process broker / fan-out pain with raw Unix Domain Socket. See `tutorial-text/Tutorial-Phase-1.md`.

---

## 🛠️ Phase 2: The Semantic Epiphany (uProtocol over a Unix Domain Socket)
*Goal: Teach uProtocol layers and constituents at tutorial depth (not overwhelming; link out to [uProtocol spec/site](https://github.com/eclipse-uprotocol/up-spec) for more). Introduce `up-unix-domain-socket-transport` implementing `UTransport`, refactor binaries around `UListener` and typed payloads — then honestly show where this design still falls short, setting up Phase 3.*

> **Scope guardrails (unchanged from planning):** one subscriber, same Unix Domain Socket path, 5 messages then exit, no Thermal crate yet, no Zenoh.

### A — Tutorial narrative (`tutorial-text/Tutorial-Phase-2.md`)
...
- [x] **2.2 Constituents in our story — what each piece does:**
  ...
  | `UTransport` | Pluggable wire layer — same API whether Unix Domain Socket or (later) Zenoh |
...
### B — Code: `up-unix-domain-socket-transport` crate

- [x] **2.4 Create `phases/02_uprotocol_semantics/crates/up-unix-domain-socket-transport`:**
  Implement `UTransport` over the existing length-framed Unix Domain Socket path (`up-frame-codec` stays; transport crate owns connect/bind, send, listener dispatch).
...
- [x] **2.5 Document benefits of `up-unix-domain-socket-transport` (in tutorial-text/Tutorial-Phase-2.md):**
...
- [x] **2.6 Document where we still fall short (Phase 3 setup):**
  Be explicit — `up-unix-domain-socket-transport` is pedagogically useful but **not** a production SDV transport:
  | Limitation | Why it matters |
  |---|---|
  | Point-to-point Unix Domain Socket | Still no fan-out; second consumer problem from Phase 1 unresolved |
...
- [x] **2.7 Refactor `up-battery-telemetry-publisher`:**
  ...
  - Publish via `SimplePublisher` + `up-unix-domain-socket-transport` (proper source/event URI)

- [x] **2.8 Refactor `up-telemetry-subscriber`:**
  - Implement `UListener` (`on_receive`) — unpack typed payload, print SoC/temperature
  - Register listener with URI filter via `up-unix-domain-socket-transport`
  - Remove inline socket-read / manual decode loop from `main`

### D — Close the phase

- [x] **2.9 Evaluate the intermediate state (tutorial + code):**
  Confirm: application routing contracts are now well-defined and declarative; **transport execution is still bottlenecked** by point-to-point Unix Domain Socket. Cliffhanger: topology change + Zenoh in Phase 3; thermal subscriber + fan-out payoff there.

- [x] **2.10 Ship Phase 2:**
  - `tutorial-text/Tutorial-Phase-2.md` (code snippets in-step, links to uProtocol docs)
  - Verify build + 5-message run
  - Commit; update `README-Notes.md` *(maintainer)*


## 🛠️ Phase 3: The Scaling Limit & The Zenoh Payoff
*Goal: Force a physical topology shift to demonstrate location transparency, swap out the Unix Domain Socket transport for Zenoh, and showcase uProtocol L3 level registration for the PUBLISH pattern.*

> **Setup (3.0 / A.1–A.5):** `phases/03_zenoh_topology/` created; `up-bms-proto` + publisher/subscriber business logic copy-forward; `up-unix-domain-socket-transport` / `up-frame-codec` remain Phase 02 only; `tutorial-text/Tutorial-Phase-3.md` skeleton. Transport wired in §3.3.

- [x] **3.1 Why we need a data-space transport (Zenoh):**
  (1) Case for Zenoh **on the same Linux host** (fan-out / Unix Domain Socket limits) even before any ECU move; (2) **thermal logger** as extra same-host benefit; (3) cross-ECU **vehicle narrative** only — demo stays one machine, no containers. See `tutorial-text/Tutorial-Phase-3.md` §3.1.
- [x] **3.2 Zenoh as data-space transport (§3.2):**
  Full narrative with 5-dimension contrast table (Unix Domain Socket vs Zenoh) and code swap walkthrough. See `tutorial-text/Tutorial-Phase-3.md` §3.2.
...
- [x] **3.4 Bring in L3 Registration for PUBLISH + thermal fan-out (§3.4):**
  Created `up-thermal-logging-subscriber` — independent binary with `ThermalLoggingListener` reading `temp_celsius` from `BatteryTelemetry`. Fan-out demo across 3 terminals (publisher + battery subscriber + thermal subscriber); no `zenohd` (peer mode on one host). See `tutorial-text/Tutorial-Phase-3.md` §3.4.
...
- [x] **3.6 Key takeaway — the Phase 3 insight (§3.6):**
  The invariant: uProtocol semantics stay constant across transport swap; Zenoh enabled fan-out without application changes. See `tutorial-text/Tutorial-Phase-3.md` §3.6 and `tutorial-text/Tutorial-Phase-1.md` cliffhanger resolution.