# Phase 2: The Semantic Epiphany — uProtocol Layers and `up-uds-transport`

> **Prerequisites:** Phase 1 (`Phase-1.md`, code in `phases/01_raw_sockets/`). Same UDS socket path, one subscriber, five messages then exit.

Phase 2 is where we stop treating uProtocol as “protobuf bytes on a wire” and start using its **layers and constituents** deliberately. We introduce **`UTransport`** over Unix Domain Sockets, refactor the publisher and subscriber around **`SimplePublisher`** / **`UListener`**, and close the RAW-payload gap with a shared **protobuf** schema — then pause honestly where UDS still fails (setup for Phase 3).
...
| No path to production transport | Raw UDS code cannot swap to Zenoh | **L1 trait boundary** — same API, different plugin in Phase 3 |

Phase 2 **builds the case**, then **shows the code**. Section **2.6** and `Phase-3.md` record what still does *not* work (fan-out, location transparency, L3) — that is intentional cliffhanger, not failure of Phase 2.

> **Authoring note:** After §2.9–2.10 ship Phase 2, consolidate this chapter's voice into `tutorial-text/Tutorial-Phase-2.md` (`Tutorial-Phase-1.md` covers Phase 1).
...
In Phase 1 we **bypassed** L1 — raw `UnixStream::read_exact` in the subscriber and `write_all` in the publisher. Phase 2 introduces `up-uds-transport` as our first real `UTransport` implementation.
...
L3 covers how entities **find and register** services across a vehicle network (e.g. PUBLISH pattern registration over a data-space transport). Phase 3 introduces this when we swap UDS for Zenoh.
...
| Constituent | What it is | Phase 1 | Phase 2 direction |
|---|---|---|---|
...
| **`UPayload`** | Application bytes + format hint | RAW CAN-style bytes | Move toward **protobuf payload** (Phase 2.7) — typed data on receive |
...
Phase 1 filled these in but the subscriber never read them. Phase 2’s transport dispatches using URI filter matching on the decoded attributes.

**Spec pointer:** [uProtocol uAttributes](https://github.com/eclipse-uprotocol/up-spec/blob/main/basics/uattributes.adoc)

### `UPayload` / `UPayloadFormat` — type enforcement (Phase 2.7)

Phase 1 showed that Protobuf types the **envelope**, not necessarily the **payload** when using `UPAYLOAD_FORMAT_RAW`. Phase 2.7 closes that gap with `BatteryTelemetry` in `up-bms-proto` (§2.7).

---

## 2.3 — Phase 1 → Phase 2 contrast

| Concern | Phase 1 (`phases/01_raw_sockets/`) | Phase 2 (`phases/02_uprotocol_semantics/`) |
|---|---|---|
...
| **Publisher / subscriber binaries** | Phase 1 style | Refactored (§2.7–2.8) |
...
Use this table when writing the tutorial or `tutorial-text/Tutorial-Phase-2.md`: it maps **removed Phase 1 code** to **what replaced it** and **why**.
...
| **Inline print/decode in spawned task** | `impl UListener { async fn on_receive(&self, msg: UMessage) }` | Testable, swappable callback; same hook Zenoh transport will invoke in Phase 3. |
...
3. **Stability across transports** — When Phase 3 swaps `UdsTransportClient` for Zenoh, the publish call site stays; only transport construction changes.
...
This crate implements uProtocol’s L1 interface over the same length-framed Unix socket path used since Phase 0 (`/tmp/uprotocol_twin.sock`). It sits between **`up-frame-codec`** (framing bytes) and the **application binaries** (publisher/subscriber).

```
┌──────────────────────────┐   UTransport::send    ┌─────────────────────────┐
│ up-battery-telemetry-    │ ────────────────────► │ up-uds-transport        │
│ publisher (Phase 2.7)     │   framed UMessage     │  UdsTransportClient     │
└──────────────────────────┘                       └───────────┬─────────────┘
                                                               │ UDS
┌──────────────────────────┐   register_listener   ┌───────────▼─────────────┐
│ up-telemetry-subscriber  │ ◄──────────────────── │ up-uds-transport        │
│ (Phase 2.8)              │   UListener dispatch  │  UdsTransport::serve    │
└──────────────────────────┘                       └─────────────────────────┘
         ▲                                                    │
         │                                                    │
...
Connects per `send` (same connect-send-close pattern as Phase 1 publisher), frames via `up-frame-codec`, and writes to the server socket.
...
Section 2.4 gave you the mechanics. This section answers the **so what**: why introduce a transport crate at all, when Phase 1 already “worked”?

The short answer: Phase 1 proved uProtocol **bytes** move over a socket. Phase 2 proves uProtocol **semantics** can sit in front of the wire — and that your application code can target a **stable L1 API** instead of re-implementing sockets in every binary.

### Benefit 1 — Application code speaks `UTransport` + `UListener`, not socket loops

In Phase 1, the subscriber’s `main` is a transport program in disguise:
...
| Role | Phase 1 (today’s binaries) | Phase 2 target (2.7–2.8) |
|---|---|---|
...
Phase 1’s subscriber accepts **every** framed message on `/tmp/uprotocol_twin.sock`. There is no declared interest — if another process connected and sent bytes, the subscriber would try to decode them. The only “filter” is accidental: one publisher, one socket, one demo.
...
- The **thermal logging engine** from Phase 1’s thought experiment could register its **own** filter URI without sharing the battery subscriber’s code path — *once* fan-out exists at the transport layer (Phase 3).
...
Phase 1 metadata (`UAttributes`, source `UUri`) was set but ignored on receive. The transport layer now **uses** that metadata for routing decisions.
...
- **Zenoh / MQTT / SOME/IP transports** — in full vehicle stacks (Phase 3 preview)
...
```
Phase 2 (this chapter)          Phase 3 (preview)
─────────────────────          ─────────────────
...
```
When we swap UDS for Zenoh, the publisher’s publish loop and the subscriber’s `on_receive` body should remain **semantically identical** — only configuration (transport plugin, endpoint) changes. Phase 1’s raw-socket code would not survive that swap without a rewrite.

### Benefit 4 — Transport concerns live in one crate, not in every binary

Phase 1 spreads transport knowledge across three places:

| Concern | Where it lives in Phase 1 |
|---|---|
...
Phase 2 consolidates the **receive-side pipeline** in `up-uds-transport`:
...
These benefits are about **API shape and separation of concerns** — not about solving Phase 1’s fan-out problem yet. We still have one socket, one subscriber process, and point-to-point UDS. Section **2.6** documents honestly where this design stops short and why Phase 3 needs a different wire.

### Checkpoint reminder

At **2.5**, the transport crate embodies these benefits, but the **demo binaries still run Phase 1 code**. Sections **2.7–2.8** wire the publisher and subscriber to `UdsTransport` so the running application matches the architecture described here.

---

## 2.6 — Where `up-uds-transport` still falls short (Phase 3 setup)

Section 2.5 argued for the transport crate on **API shape** grounds. This section is the honest counterweight: **`up-uds-transport` is pedagogically useful, not production SDV transport.** It teaches uProtocol L1 on a wire you already understand; it does **not** remove the architectural wall from Phase 1.

That wall matters because real vehicles add services continuously. Remember the **Thermal Management Logging Engine** from `Phase-1.md`: an independent microservice that must monitor the same battery temperature stream without sharing the battery subscriber's process or internal state. Phase 2 improves how we *express* interest in that stream (URI filters, `UListener`) — but the **wire underneath is still a single local Unix socket with point-to-point semantics**.

```
Phase 1 wall                         Phase 2 (2.4–2.5)                    Still blocked
────────────────                     ─────────────────                    ─────────────
...
|---|---|
| **Point-to-point UDS** | A `SOCK_STREAM` connection has one reader; bytes are consumed on read. No native fan-out — the Phase 1 second-consumer problem is **unresolved**. |
...
Each row is intentional. Phase 2 fixes **how application code talks to transport**; Phase 3 must fix **where and how messages propagate**.
...
```
That is a real improvement over Phase 1's ad-hoc decode loop — but it is **not** network pub/sub. A second **process** (the thermal logger) cannot call `register_listener` on your subscriber's server handle. If it opened its own listener on the same socket path, it would **compete** for the publisher's connection, not share the stream.

Phase 1's pseudo-code broker trap (`Phase-1.md`) — subscriber becomes fan-out hub, manual stream lists, back-pressure in the hot path — remains the only UDS-native escape hatch. **`up-uds-transport` does not implement that broker**, and we should not pretend URI filters magically create multi-process fan-out. Filters route **within** a transport instance; they do not clone bytes to arbitrary peers.

**Phase 3 payoff:** a data-space transport (Zenoh) provides native multi-subscriber delivery so the thermal engine can subscribe independently — without turning the battery subscriber into a broker.
...
Contrast with Phase 3's direction: configure a **Zenoh-backed** `UTransport` with session endpoints; business logic still calls `send` and `register_listener`, but the wire resolves peers through the data space instead of a path baked into `main`.

```
Phase 2 (today)                          Phase 3 (preview)
─────────────────                        ─────────────────
...
Phase 2.1 previewed L3; here is what we **do not** have yet:

| Mechanism | Phase 2 (`up-uds-transport`) | Production uProtocol stack |
|---|---|---|
...
### What Phase 3 changes vs what Phase 2 keeps

| Layer | Phase 2 (keep) | Phase 3 (replace wire) |
|---|---|---|
...
**Phase 3 replaces transport execution; Phase 2 semantics stay.** That sentence is the bridge to the next chapter: you are not relearning uProtocol — you are **unblocking topology**. Full handoff notes live in **`Phase-3.md`** (planning brief for the Phase 3 chapter).
...
Phase 1 packed SoC and temperature into a fictional CAN byte layout (`pack_bms_can_frame`) and marked the payload `UPAYLOAD_FORMAT_RAW`. That worked for a demo, but it recreated the **shared secret** problem: meaning lived outside the uProtocol envelope.

Phase 2.7 closes the gap with a **protobuf application payload** and routes sends through **`SimplePublisher`** + **`UdsTransportClient`**.
...
| **Phase 1 contrast** | RAW bytes required a DBC-style secret; protobuf makes the payload **self-describing** via `UPayloadFormat`. |
| **Scope guardrail** | Only battery SoC and cell temperature here — thermal logging and other consumers arrive in Phase 3. |
...
| Concern | Phase 1 subscriber | Phase 2.8 subscriber |
|---|---|---|
...
```
The accept/dispatch loop keeps running in the transport crate's background task — we simply **exit the process** once business logic has seen five messages (scope guardrail unchanged from Phase 1).
...
## 2.9 — Evaluate the intermediate state (Phase 2 synthesis)

Section 2.9 is the **chapter close** for Phase 2. It does not add crates; it states what a reader should believe after running Phase 2 code.
...
### What Phase 2 still cannot do (input for Phase 3)
...
See **`Phase-3.md`** for the structured author brief (topology trigger, migration sketch, roadmap links).

### Reader takeaway (for `tutorial-text/Tutorial-Phase-2.md`)

> Phase 2 is not “UDS with extra steps.” It is **learning which uProtocol layer to implement yourself** versus **which layer the stack provides**. You implemented battery logic; uProtocol L1/L2 implemented routing contracts and envelope discipline. The wire is still wrong for a vehicle — **on purpose** — so Phase 3’s Zenoh swap has something to prove.

---

## 2.10 — Ship Phase 2

Phase 2 is **code- and narrative-complete** in `phases/02_uprotocol_semantics/`. Run it directly from the repo root — no git checkout needed.
...
Consolidate Phase 2 prose into **`tutorial-text/Tutorial-Phase-2.md`** (`Tutorial-Phase-1.md` remains Phase 1). Planning brief for Phase 3 code: **`Phase-3.md`**.

---

## Key takeaway at Phase 2

Phase 2 completes the **semantic** story: routing contracts are well-defined, payloads are typed, and application code targets **`UTransport` + `UListener` + L2 publish**. **Transport execution** is still bottlenecked by point-to-point UDS — documented in §2.6 and **`Phase-3.md`**. That split is the deliberate chapter ending: improve the API surface first, swap the wire in Phase 3 without rewriting battery logic.
