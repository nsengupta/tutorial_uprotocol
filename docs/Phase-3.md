...
> **Prerequisites:** Phase 2 (`docs/Phase-2.md`, code in `phases/02_uprotocol_semantics/`).  
> **Active code:** `phases/03_zenoh_topology/` — copy-forward from Phase 2.

Phase 3 addresses everything Phase 2 **documented but did not fix** — starting with problems that appear **even on a single Linux host** (fan-out, broker semantics), then the **thermal logger payoff**, and finally the **cross-ECU story** real vehicles face (narrative only in this tutorial). The code payoff: **Phase 2 business logic survives** — only the wire and discovery story change.

> **Demo scope (Phase 3):** All processes run on **one Linux machine** (separate terminals), same as Phases 1–2. We do **not** use multiple hosts, VMs, or containers to simulate ECUs. The Zone ECU / central compute split is **vehicle narrative** — it explains an *extra* benefit of Zenoh, not something we reproduce in the lab demo.
...
|---|---|---|
| **Status** | Frozen in `phases/02_uprotocol_semantics/` | Active development |
...
3. **Copy-forward snapshots** — Earlier phases stay under `phases/` so readers can diff chapters. Phase 2 code remains in `phases/02_uprotocol_semantics/`; Phase 3 starts clean without dragging UDS plumbing into the Zenoh story.
...
| Phase 2 gap (same host) | Phase 3 response |
|---|---|
...
| Phase 2 gap (cross-ECU — narrative) | Phase 3 response |
|---|---|
...
These crates **taught Phase 2 well** but **must not appear** in the Phase 3 active workspace:

| Crate | Role in Phase 2 | Why retired in Phase 3 |
|---|---|---|
...
They remain in `phases/02_uprotocol_semantics/` for side-by-side study — not copied, not depended on, not imported from Phase 02 into Phase 03 binaries.
...
Phase 2 left us on **one Linux host** with a working 1:1 demo. Phase 3 asks a harder question first:
...
| Same pattern as Phases 1–2: **two or three terminals**, five messages, exit | Physical Zone ECU hardware |
...
Phases 1–2 used `/tmp/uprotocol_twin.sock`. That worked for **one publisher, one subscriber, one process each** — a deliberately narrow demo. It does **not** scale to how services actually attach in an SDV stack, **even when every binary runs on the same kernel**.
...
Same Linux host — Phase 2 UDS (still broken for fan-out)
  ...
  up-thermal-logging-subscriber (§3.4)
         ...
         └── cannot share the stream — second process has no socket to read
             (Phase 1 broker pseudo-code trap — docs/Phase-1.md)
```
...
**You do not need a second machine to hit this wall.** Phase 1 introduced the thermal logging engine as a **second consumer on the same vehicle**; Phase 2 made URI interest declarative but **did not deliver bytes to a second process**. That alone justifies retiring UDS for Phase 3 — before anyone moves an ECU.
...
Phase 2's **`UUri`** (entity `0x1010`, resource `0x8001`) is the logical contract. Zenoh (and L3 in §3.4) align **wire delivery** with that contract. UDS aligned it only for a single pipe on a path string.
...
| Approach | Problem |
|---|---|
| Extend `up-uds-transport` with fan-out lists | Reinvents a message broker inside application code (Phase 1 trap) |
...
Once Zenoh replaces UDS, the **Phase 1 narrative payoff** lands **without moving any process to another machine**:
...
```
| Consumer | Process | Cares about | Phase 2 UDS | Phase 3 Zenoh (same host) |
|---|---|---|---|---|
...
The thermal crate does **not** require cross-ECU topology. It requires **fan-out** — which UDS could not provide on one host. That is the **extra benefit of Zenoh in this tutorial**: the second service from `docs/Phase-1.md` finally attaches cleanly, in a **third terminal**, while the battery subscriber code stays untouched.
...
| Before (Phases 1–2) | Vehicle story (narrative) |
|---|---|
...
Wire the **Thermal Management Logging Engine** (introduced in Phase 1 §1.4) as an independent binary — no socket sharing with the battery subscriber.
...
Yes — both the battery telemetry subscriber and the thermal logging subscriber received all 5 messages. Each subscriber runs in its own process, connected to the same Zenoh router. No UDS socket path; no shared filesystem. Zenoh delivers the publication to every subscriber whose URI filter matches — that is the fan-out that Phase 2's point-to-point UDS could not provide.
...
| `docs/Phase-3.md` complete | All sections §3.0–§3.6 filled | ✅ Done |

Phase 3 code lives in `phases/03_zenoh_topology/` — run directly with `--manifest-path`; no git checkout needed.

---

## Handoff reference (from Phase 2)

What Phase 2 delivered and Phase 3 must preserve:
...
What Phase 2 could not fix (Phase 3 drivers):
...
See `docs/Phase-2.md` §2.6 for full prose.

---

## Key takeaway at Phase 3

Phase 3 proves that **uProtocol semantics decouple application logic from transport topology**. Phase 2 built the right abstractions; Phase 3 swaps UDS for Zenoh — first to unlock **fan-out on one machine** (thermal logger), then (in production stories) **cross-ECU** delivery without rewriting battery logic.
...
**Retirement recap:** `up-uds-transport` and `up-frame-codec` belong to the **Phase 2 UDS story**, frozen under `phases/02_uprotocol_semantics/`. They are intentionally absent from Phase 03 — not because they were wrong for a 1:1 demo, but because **even a second subscriber on the same host** outgrows point-to-point UDS. What survives is everything above L1: protobuf payloads, URI filters, `SimplePublisher`, and `UListener`.
